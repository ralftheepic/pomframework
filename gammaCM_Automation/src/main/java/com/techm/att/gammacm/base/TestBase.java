...
public class TestBase extends CommonTestActions {
	...
	public static String dateAndTime = new SimpleDateFormat("dd-MMM-yyy hh-mm a z")
	...
	public static String screenshotsDirectory = System.getProperty("user.dir") + "\\Screenshots\\Screenshots" + "_"
			+ dateAndTime;
	...
	public void initializeSuite() {
		try {
			...
			reports = new ExtentReports(
					System.getProperty("user.dir") + "\\TestResults\\Gamma_CM_TestAutomationReport.html");
	...
	public void initDriver() {
		try {
			String browser = System.getProperty("browser.type") != null && !System.getProperty("browser.type").isEmpty()
					? System.getProperty("browser.type") : browserType;
	...
	/*
	 * private void initPhantomDriver(String appURL) { logger.info(
	 * "Launching browser in headless mode via PhantomJS......");
	 * System.setProperty("phantomjs.binary.path",
	 * System.getProperty("user.dir")+"\\drivers\\phantomjs.exe"); driver = new
	 * PhantomJSDriver(); setDriver(driver);
	 * getDriver().manage().window().maximize(); getDriver().get(appURL);
	 * getDriver().manage().timeouts().implicitlyWait(
	 * DEFAULT_IMPLICIT_WAIT_TIME, TimeUnit.SECONDS); logger.info(
	 * "Launched URL : " + appURL); }
	 */
	...
	public void validateIfTestCaseAllowed(String methodName) throws IOException {
		if (TestCases.contains(methodName.trim())) {
			...
			getCurrentLogger().log(LogStatus.INFO,
					"Skipping the test '" + methodName + "' as testcase Runmode is set to: NO");
	...
	public void tearDown(ITestResult result) throws InterruptedException {
		if (reports != null && getCurrentLogger() != null) {
			if (result.getStatus() == ITestResult.SKIP) {
				...
				// String image =
				// getCurrentLogger().addScreenCapture(screenShotPath);
				...
				// }else{
				// logger.info(result.getName() + " Test Script is Passed when
				// re-Excuted");
				// getCurrentLogger().log(LogStatus.INFO, result.getName() + "
				// Test Script is Passed when re-Excuted");
				// }
